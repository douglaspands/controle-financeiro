...
ECMA_ONKEYUP = (r'(e=>{const s=e.value.match(/[0-9]/g).join("");if(s){const[t,i]=(parseInt(s, 10)/100).toFixed(2).split("."),u=t.split("").reverse().reduce((e,s,t)=>(t<=1?e.push(s):(t+1)%3==1?e.push(`${s}THOUSAND_POINT`):e.push(s),e),[]).reverse().join("");e.value=`${u}DECIMAL_POINT${i}`}else e.value=""})(this);'
...
class MonetaryField(forms.Field):
    def __init__(self, *args, **kwargs):
        kwargs["widget"] = forms.TextInput(attrs={"onkeyup": ECMA_ONKEYUP, "maxlength": 20})
